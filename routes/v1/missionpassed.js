...
router.post('/missionpassed', (req, res) => {
    try {
        ...
        Promise.all([APIConstants.Image[0], APIConstants.Image[1]]).then((images) => {
            ...
            images[0].resize(iUploadedPicWidth, iUploadedPicHeight).greyscale()
                .composite(images[1].resize(iUploadedPicWidth, iUploadedPicHeight), 0, 0)
                .quality(100).getBuffer(Jimp.MIME_PNG, (err, buffer) => {
                    if (err) {
