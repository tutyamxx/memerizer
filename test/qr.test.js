...
describe("🎯 Testing (QR Code) API endpoint", () =>
{
    ...
    it("GET - Should return a message and a status code of (400) if there is no query specified", (done) =>
    {
        ...
        .end((err, response) =>
        {
            ...
            expect(response.body).to.have.a.property("message").and.to.be.a("string").and.to.be.equal("Invalid text query specified! Correct usage is /qr?text='your text here' 🙄");
    ...
    {
        request(app).get("/api/v1/qr?text=")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(400);
    ...
    {
        request(app).get("/api/v1/qr?text=testing")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(400);
    ...
    {
        request(app).get("/api/v1/qr?text=testing&format=nope")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(400);
    ...
    {
        request(app).get("/api/v1/qr?text=testing&format=buffer").end((err, response) =>
        {
    ...
    {
        request(app).get("/api/v1/qr?text=testing&format=base64").end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
