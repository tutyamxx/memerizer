...
describe("🎯 Testing (Yu-Gi-Oh! Trap Card) API endpoint", () =>
{
    it("GET - Should return a message and a status code of (404) for GET requests", (done) =>
    {
        request(app).get("/api/v1/trapcard")
    ...
    {
        request(app).post("/api/v1/trapcard")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/trapcard?format=nope")
        .expect("Content-Type", /json/)
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/trapcard?format=buffer")
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/trapcard?format=base64")
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
