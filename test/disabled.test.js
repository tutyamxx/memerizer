...
describe("🎯 Testing (Disabled) API endpoint", () =>
{
    ...
    {
        request(app).post("/api/v1/disabled")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/disabled?format=nope")
        .expect("Content-Type", /json/)
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/disabled?format=buffer")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/disabled?format=base64")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
