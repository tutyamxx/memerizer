...
describe("🎯 Testing (Pixel) API endpoint", () =>
{
    ...
    {
        request(app).post("/api/v1/pixel?format=nope")
        .expect("Content-Type", /json/)
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/pixel?format=buffer")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/pixel?format=base64")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
