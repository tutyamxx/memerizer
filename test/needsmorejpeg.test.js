...
describe("🎯 Testing (Needs more JPEG) API endpoint", () =>
{
    ...
    {
        request(app).get("/api/v1/needsmorejpeg")
    ...
    {
        request(app).post("/api/v1/needsmorejpeg")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/needsmorejpeg?format=nope")
        .expect("Content-Type", /json/)
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/needsmorejpeg?format=buffer")
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/needsmorejpeg?format=base64")
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
