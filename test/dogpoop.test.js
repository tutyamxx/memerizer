...
describe("🎯 Testing (Dogpoop) API endpoint", () =>
{
    ...
    {
        request(app).post("/api/v1/dogpoop")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/dogpoop?format=nope")
        .expect("Content-Type", /json/)
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/dogpoop?format=buffer")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/dogpoop?format=base64")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
