...
describe("🎯 Testing (Beautiful) API endpoint", () =>
{
    ...
    {
        request(app).post("/api/v1/beautiful")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
    ...
    it("POST - Should return a message and a status code of (400) if a valid image URL type is attached, but the `format` query contains an invalid format", (done) =>
    {
        request(app).post("/api/v1/beautiful?format=nope")
        .expect("Content-Type", /json/)
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/beautiful?format=buffer")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/beautiful?format=base64")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
