...
describe("🎯 Testing (Worse than hitler Family Guy) API endpoint", () =>
{
    it("GET - Should return a message and a status code of (404) for GET requests", (done) =>
    {
        request(app).get("/api/v1/worsethan")
    ...
    {
        request(app).post("/api/v1/worsethan")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/worsethan?format=nope")
        .expect("Content-Type", /json/)
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/worsethan?format=buffer")
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/worsethan?format=base64")
        .attach("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
