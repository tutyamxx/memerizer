...
describe("🎯 Testing (Draw) API endpoint", () =>
{
    ...
    {
        request(app).post("/api/v1/draw?format=nope")
        .expect("Content-Type", /json/)
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/draw?format=buffer")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/draw?format=base64")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
