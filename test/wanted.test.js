...
describe("🎯 Testing (Wanted) API endpoint", () =>
{
    ...
    {
        request(app).post("/api/v1/wanted")
        .expect("Content-Type", /json/)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/wanted?format=nope")
        .expect("Content-Type", /json/)
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/wanted?format=buffer")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/wanted?format=base64")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
