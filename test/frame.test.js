...
describe("🎯 Testing (Frame) API endpoint", () =>
{
    ...
    {
        request(app).post("/api/v1/frame?format=nope")
        .expect("Content-Type", /json/)
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/frame?format=buffer")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
    ...
    {
        request(app).post("/api/v1/frame?format=base64")
        .field("image", FileImageCat)
        .end((err, response) =>
        {
            expect(response.status).to.be.equal(200);
